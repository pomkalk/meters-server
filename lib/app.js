...
const waitFor = (s) => new Promise(resolve=>setTimeout(resolve, s))
        ...
        try {
            ...
            this.apiRouter.post('/api/get-token/qr', async (ctx) => {
                try {
                    ...
                    let res
                    if (body.expo) {
                        [res] = await this.sql`INSERT INTO tokens ${ this.sql({ ls: apartment.ls, did: body.did, expo: body.expo, notify: true }) } RETURNING *`
                    } else {
                        [res] = await this.sql`INSERT INTO tokens ${ this.sql({ ls: apartment.ls }) } RETURNING *`
                    }
            ...
            this.apiRouter.post('/api/get-token', async (ctx) => {
                try {
                    ...
                    const space = parseFloat(data.space.toString().replace(',','.'))
                    let [apartment] = await this.sql`SELECT a.id, a.ls, a.space, 
                        CONCAT(s.type, '. ', s.name, ', д. ', CONCAT_WS('/', b.number, NULLIF(b.housing, '')), ', кв. ', CONCAT_WS('/', a.number, NULLIF(a.part, ''))) as address 
                        FROM apartments AS a
                        LEFT JOIN buildings AS b ON b.id=a.building_id
                    ...
                    let token
                    if (data.remember || data.expo) {
                        let res
                        if (data.expo) {
                            [res] = await this.sql`INSERT INTO tokens ${ this.sql({ ls: apartment.ls, did: data.did, expo: data.expo, notify: true }) } RETURNING *`
                        } else {
                            [res] = await this.sql`INSERT INTO tokens ${ this.sql({ ls: apartment.ls }) } RETURNING *`
                        }
                        td.tid = res.id
            ...
            this.apiRouter.post('/api/get-meters', async (ctx) => {
                try {
                    ...
                    let [[address], meters, [dbtoken]] = await Promise.all([
                        ...
                        this.sql`SELECT * FROM tokens WHERE id=${tid}`
                    ])
                    res.address = address.address
                    res.meters = meters
                    if (dbtoken) {
                        if (dbtoken.expo) {
                            res.notifications = dbtoken.notify
                        }
                    }
                    this.log.info(`[${ls}] <OPEN> ${address.address}`)
                    ctx.body = res
                } catch (e) {
                    this.log.error(e, ctx.request.body)
                    ctx.staus = 500
            ...
            this.apiRouter.post('/api/save', async (ctx) => {
                try {
                    ...
                    let { ls, tid, mob } = jwt.verify(token, env.secret)
                    let [r] = await this.sql`
                        SELECT m.*, p.p_start, p.p_end FROM meters AS m 
                        LEFT JOIN periods AS p ON p.id=m.period_id
                        WHERE m.id=${meter_id} AND m.ls=${ls}`

                    console.log(r)
                    //let [meter] = await this.sql`SELECT * FROM meters WHERE id=${meter_id} AND ls=${ls}`
                    //let [period] = await this.sql`SELECT * FROM periods WHERE id = ${meter.period_id}`
                    ...
                    // ])
                    let current = new Date()
                    let start = new Date(r.p_start)
                    let end = new Date(r.p_end)
                    let access = current>start&&current<end
                    ...
                    if (r.service===1||r.service===2) {
                        let m = value - r.last_value
                        if (m>30) {
                            ctx.body = {error: `Слишком большое значени показаний. Максимальное значение для данного счетчика может быть ${r.last_value+30}`}
                            return
                        }
                        if (value < r.last_value && value!==null) {
                    ...
                    let [meter2] = await this.sql`UPDATE meters SET new_value=${value}, src=${src}, new_date=NOW() WHERE id=${meter_id} RETURNING *`
                    ...
                    ctx.body = {
                        meter: {
                            id: meter2.id,
                            service: meter2.service,
                            status: meter2.status,
                            last_month: meter2.last_month,
                            last_year: meter2.last_year,
                            last_value: meter2.last_value,
                            new_value: meter2.new_value,
                            new_date: meter2.new_date
                        }
                    }
                } catch (e) {
                    console.log('suck')
            ...
            this.apiRouter.post('/api/device/reg', async (ctx) => {
                try {
                    let device = ctx.request.body
                    let [d] = await this.sql`SELECT * FROM devices WHERE did=${device.did}`
                    console.log(d)
                    if (d) {
                        await this.sql`UPDATE devices SET expo=${device.expo}, info=${this.sql.json(device.info||null)} WHERE did=${device.did}`
                    } else {
                        await this.sql`INSERT INTO devices (did, expo, info, news) VALUES (${device.did}, ${device.expo}, ${ this.sql.json(device.info||null)}, ${device.news|| true})`
                    }
                    ctx.body = 'ok'
                } catch (e) {
                    this.log.error(e, ctx.request.body)
            ...
            this.apiRouter.get('/api/get-news', async (ctx) => {
                try {
                    let page = ctx.request.query.page || 1
                    const [[total], data] = await Promise.all([
                        this.sql`SELECT COUNT(*) FROM news WHERE deleted_at IS NULL`,
                        this.sql`SELECT n.id, n.title, n.body, n.created_at
                            FROM news as n
            ...
            this.apiRouter.post('/api/notifications', async (ctx) => {
                try {
                    const token = ctx.request.body.token
                    const notify = ctx.request.body.notify
                    let { ls, tid } = jwt.verify(token, env.secret)
                    await this.sql`UPDATE tokens SET notify=${notify?true:false} WHERE id=${tid}`
                    ctx.body = 'ok'
                } catch (e) {
                    this.log.error(e, ctx.request.body)
                    ctx.staus = 500
...
// SELECT * FROM meters WHERE id=850314 AND ls=103030;
// SELECT * FROM periods WHERE id = 12;
// UPDATE meters SET new_value=130, src=0, new_date=NOW() WHERE id=850314 RETURNING *;
