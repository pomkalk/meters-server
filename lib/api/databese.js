...
class DatabaseApi extends BaseApi {
    ...
    async download(req, res) {
        try {
            let data = await this.sql`SELECT m.id, m.ls, m.mid, CONCAT(s.type, '. ', s.name, ', д. ', 
            ...
            WHERE m.period_id=${req.period_id} AND m.src IS NOT NULL
    ...
    async deletePeriod (period_id) {
        try {
            ...
            if (last.month===current.month&&last.year===current.year) {
                await this.sql`UPDATE periods SET deleted_at=NOW() WHERE id=${period_id}`
