        ...
        try {
            ...
            if (last.month===current.month&&last.year===current.year) {
                await this.sql.begin(async sql=> {
                    ...
                    console.log('#', 4)
                    const [count] = await sql`SELECT COUNT(*) FROM meters_history WHERE period_id=${period.id}`
                    console.log('#', 5)
                    console.log(period.id, count)
                    if (count.count === 0 ) {
                        console.log('x1')
            ...
            await this.sql.begin(async sql => {
                ...
                if (streets.update.length > 0) {
                    for (let x of streets.update) {
                        ...
                        this.log.info(`Streets updated: $${x.id}: ${x.type}. ${x.name}`)
                    }
                }
                ...
                const [old_period] = await sql`SELECT period_id as id from meters LIMIT 1`
                const [count] = await sql`SELECT COUNT(*) FROM meters_history WHERE period_id=${old_period.id}`
                if (count.count === 0) {
                    await sql`INSERT INTO meters_history SELECT * FROM meters`
                } else {
                    const update_data = await sql`
                        SELECT m.*
                        FROM meters AS m, meters_history AS mh
                        WHERE m.id=mh.id AND m.new_value IS NOT NULL AND m.new_value != mh.new_value`
                    for (let x of update_data) {
                        await sql`UPDATE meters_history SET ${ sql(x) } WHERE id=${x.id}`
                        this.log.info(`meter history updated: ${x.id}`)
                    }
                }
