    ...
    async getFeedbacks (params) {
        try {
            const [[total], data] = await Promise.all([
                ...
                this.sql`SELECT f.id, f.ls, left(f.body, 40)||'...' as body, f.read, f.created_at, CONCAT(s.type, '. ', s.name, ', д. ', CONCAT_WS('/', b.number, NULLIF(b.housing, '')), ', кв. ', CONCAT_WS('/', a.number, NULLIF(a.part, ''))) as address
                FROM feedbacks AS f
                LEFT JOIN apartments AS a ON a.ls=f.ls
                LEFT JOIN buildings AS b ON b.id=a.building_id
                LEFT JOIN streets AS s on s.id=b.street_id
