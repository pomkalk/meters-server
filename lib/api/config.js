...
class ConfigApi extends BaseApi {
    ...
    async getUsers () {
        try {
            ...
            const online_ids = Object.keys(this.app.users)
            console.log(online_ids)
            ...
            users = users.map(user => {
                ...
                console.log(user.id)
                if (online_ids.includes(user.id.toString())) {
                    user.online = true
                }
            ...
            this.emitError('Ошибка при получении данных пользователей.')
        }
    ...
    async getConfig () {
        try {
            ...
            this.log.error('get config error', e)
            this.emitError('Ошибка при получении данных конфигурации.')
        }
    }
    ...
    async setConfig (data) {
        try {
            ...
            this.emitError('Ошибка при сохранении данных.')
        }
    }
}
